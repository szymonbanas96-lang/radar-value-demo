...
def _format_rows(rows):
    ...
    df["date_label"] = local.dt.strftime("%a, %d %b").str.upper()
...
def _fallback():
    return _format_rows([{
        "datetime_utc":dt,"away":away,"home":home,"venue":venue,
        "special":special,"status":"Upcoming"
...
def _fetch_espn():
    ...
    while day <= end:
        r = requests.get(
            ...
            headers={"User-Agent":"RadarValue/0.7"},
            timeout=5,
        )
        r.raise_for_status()
    ...
    if not rows:
        raise RuntimeError("No schedule data")
    return _format_rows(rows).drop_duplicates(subset=["datetime_utc","away","home"]).reset_index(drop=True)

def get_preseason_schedule():
    try:
        return _fetch_espn(), "live schedule feed", "Times shown in Europe/Warsaw."
    except Exception:
        return _fallback(), "NBA official fallback schedule", "Live feed unavailable, bundled schedule displayed."
